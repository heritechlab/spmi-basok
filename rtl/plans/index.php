...
$statistics = $service->getActionPlanStatistics($myUnitId);
...
<style>
    #tablePlans { font-size: 12.5px; border-collapse: separate; border-spacing: 0; }
    #tablePlans thead th {
    ...
    #tablePlans tbody tr:nth-child(even) { background: #faf9fd; }
    #tablePlans tbody tr:hover { background: #f1edfc; }
    #tablePlans tbody td:first-child { border-left: 3px solid #ddd0f7; font-weight: 600; color: #14112b; }
    #tablePlans tbody tr:hover td:first-child { border-left-color: #7c3aed; }
    #tablePlans .badge { font-weight: 600; font-size: 10.5px; padding: 4px 10px; border-radius: 20px; }
    #tablePlans .btn-sm { border-radius: 8px; padding: 5px 10px; font-size: 11px; }
    ...
    .card:has(#tablePlans) .card-body { padding: 20px; }

    .card:has(#tablePlans) .form-select,
    .card:has(#tablePlans) .form-control {
        font-size: 12.5px; font-weight: 500; color: #2d2a45;
        border: 1px solid #e2dff2; border-radius: 8px; padding: 8px 12px;
        transition: border-color .15s, box-shadow .15s;
    }
    .card:has(#tablePlans) .form-select:focus,
    .card:has(#tablePlans) .form-control:focus {
        border-color: #7c3aed; box-shadow: 0 0 0 3px rgba(124,58,237,0.10); outline: none;
    }
    .card:has(#tablePlans) .form-label {
...
<div class="container-fluid py-4">
 ...
 <h4 class="mb-3">Penetapan RTL</h4>
 ...
 <?php require_once __DIR__ . '/../tabs.php'; ?>
    ...
    <div class="dual-card-row mb-3">
        ...
        <div class="greeting-card">
            ...
            <div class="indicator-summary-greeting">
                Berikut rekap seluruh Rencana Tindak Lanjut (RTL) yang telah ditetapkan.
            </div>
        ...
        <div class="info-card info-card-compact">
            ...
            <div class="info-card-body">
                ...
                <?php
                    $totalRtl = $statistics['data']['total'] ?? 0;
                    $pctBelum = $totalRtl > 0 ? round((($statistics['data']['belum'] ?? 0) / $totalRtl) * 100) : 0;
                    $pctProses = $totalRtl > 0 ? round((($statistics['data']['proses'] ?? 0) / $totalRtl) * 100) : 0;
                    $pctSelesai = $totalRtl > 0 ? round((($statistics['data']['selesai'] ?? 0) / $totalRtl) * 100) : 0;
                ?>
                ...
                <div class="info-card-items-grid info-card-items-grid-3">
                    ...
                    <div class="info-card-item accent-purple">
                        ...
                        <div class="info-card-text">
                            ...
                            <div class="info-card-value count-up" data-target="<?= $statistics['data']['belum'] ?? 0 ?>">0</div>
                            <div class="info-card-progress-track">
                                <div class="info-card-progress-fill" data-percent="<?= $pctBelum ?>"></div>
                            </div>
                        </div>
                    </div>

                    <div class="info-card-item accent-orange">
                        <div class="info-card-icon"><i class="bi bi-arrow-repeat"></i></div>
                        <div class="info-card-text">
                            <div class="info-card-label">Proses</div>
                            <div class="info-card-value count-up" data-target="<?= $statistics['data']['proses'] ?? 0 ?>">0</div>
                            <div class="info-card-progress-track">
                                <div class="info-card-progress-fill" data-percent="<?= $pctProses ?>"></div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            ...
            <table class="table table-bordered table-hover" id="tablePlans">
...
<script src="<?= BASE_URL ?>assets/js/rtm_plans.js"></script>