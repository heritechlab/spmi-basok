...
$auditors = $auditorRepo->getAll('', $status, 1000, 0);
...
<head>
    <meta charset="UTF-8">
    <title>Daftar Auditor - <?= htmlspecialchars($profile['institution_name'] ?? 'SIQUA') ?></title>
...
<body>
    ...
    <table>
        ...
        <tbody>
            ...
            <?php else: ?>
                <?php foreach ($auditors as $i => $row): ?>
                    <tr>
                        <td style="text-align:center;"><?= $i + 1 ?></td>
                        <td><?= htmlspecialchars($row['full_name']) ?></td>
                        <td><?= htmlspecialchars($row['nidn_nip'] ?? '-') ?></td>
                        <td><?= htmlspecialchars($row['email'] ?? '-') ?></td>
                        <td><?= htmlspecialchars($row['phone'] ?? '-') ?></td>
                        <td style="text-align:center;">
                            <?= (int)$row['status'] === 1 ? 'Aktif' : 'Nonaktif' ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <div class="signature">
        <p>
            <?= htmlspecialchars($profile['city'] ?? '') ?>, <?= date('d F Y') ?>
        </p>
