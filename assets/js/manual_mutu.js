const ManualMutu = {
  api: SIQUA.BASE_URL + "manual_mutu/api.php",
  uploadModal: null,
};

$(document).ready(function () {
  ManualMutu.uploadModal = new bootstrap.Modal(
    document.getElementById("uploadModal"),
  );

  loadList("manual_ppepp", "#listManualPpepp");
  loadList("manual_lainnya", "#listManualLainnya");
...
ManualMutu.openUpload = function (category) {
  $("#uploadForm")[0].reset();
  $("#upload_category").val(category);
  $("#uploadModalTitle").text(
    category === "manual_ppepp"
      ? "Unggah Manual PPEPP"
      : "Unggah Manual Mutu Lainnya",
  );
  ManualMutu.uploadModal.show();
};
...
function submitUpload() {
  ...
  $.ajax({
    url: ManualMutu.api + "?action=upload",
    type: "POST",
    data: formData,
    processData: false,
    contentType: false,
    ...
    success: function (response) {
      ...
      loadList(
        category,
        category === "manual_ppepp" ? "#listManualPpepp" : "#listManualLainnya",
      );
    },
    error: function () {
      Swal.close();
...
function deleteDocument(id, category) {
  Swal.fire({
    ...
    $.post(ManualMutu.api + "?action=delete", { id: id }, function (response) {
      if (!response.success) {
        Swal.fire("Gagal", response.message, "error");
        return;
      }
